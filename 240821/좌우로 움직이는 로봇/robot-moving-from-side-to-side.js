...
while(aMoving.length > 0 || bMoving.length > 0 || aSecond > 0 || bSecond > 0){
    if(aMoving.length !== 0 && aSecond === 0){
        aLine = aMoving.splice(0, 1)[0].split(" ");  // splice로 첫 번째 요소를 제거하고 반환
        [aSecond, aOrder] = [parseInt(aLine[0]), aLine[1]];
    }
    
    if(bMoving.length !== 0 && bSecond === 0){
        bLine = bMoving.splice(0, 1)[0].split(" ");  // splice로 첫 번째 요소를 제거하고 반환
    ...
    let count = (aSecond !== 0 && bSecond !== 0) ? Math.min(aSecond, bSecond) : Math.max(aSecond, bSecond);
